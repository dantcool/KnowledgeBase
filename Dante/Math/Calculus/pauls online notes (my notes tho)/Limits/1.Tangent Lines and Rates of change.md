...
To compute average rate of change of f(x) at x=a just choose another point say x then the average rate of change will be 


A.R.C = $\frac{change in f(x)}{change in x}$
=$\frac{f(x)-f(a)}{x-a}$
...
1st step get a formula for the average rate of change of the volume in this case would be 

A.R.C. $=\frac{V(t)-V(5)}{t-5} = \frac{t^3-6t^2+35-10}{t-5} = \frac{t^3-6t^2+25}{t-5}$

2nd step since we are looking for the instantaneous rate of change of the volume at t=5 plug in values that are getting closer and close to 5.
...
from the table we can see that the average rate of change is apporaching 15 so we can estimate that the instantaneous rate of change is 15


## Velocity Problem 
to estimate instantaneous velocity we would first compute average velocity 

A.V = $\frac{Change in positon}{Time traveled}$
= $\frac{f(t)-f(a)}{t-a}$

then take values of t closer and closer to t=a and use the values to estimate instantaneous velocity 
